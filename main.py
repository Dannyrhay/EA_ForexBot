...
class TradingBot:
    def __init__(self):
        ...
        self.recent_signals = [] # [NEW] Buffer for UI signals
        self.recent_signals_lock = threading.Lock() # [NEW] Lock for signal buffer
    ...
    def can_open_new_trade(self, symbol, strategy_name=None):
        ...
        if self.risk_manager:
            ...
            if acc_info:
                ...
                if not self.risk_manager.check_margin_level(acc_info):
                    return False
                
                # 2. Check Daily Drawdown
                if not self.risk_manager.check_daily_drawdown(acc_info.equity, self.daily_starting_equity):
                    return False
                
                # 3. Check Portfolio Risk (Estimate new trade risk? For now, check existing)
                # Ideally we check if adding a NEW trade would breach. 
                # Since we don't know the exact risk of the new trade yet (SL not calc'd), 
                # we can pass 0 to check if we are ALREADY breached, or pass a conservative estimate.
                # Let's check existing first.
                positions = mt5.positions_get()
                if not self.risk_manager.check_portfolio_risk(positions, acc_info.equity, new_trade_risk_amount=0):
                    return False

        positions = mt5.positions_get(symbol=symbol)
        if positions:
            # If strategy_name is provided, check if THIS strategy already has a trade
            if strategy_name:
                for pos in positions:
                    # Check comment for strategy name (format: AI-{strategy}-{timeframe})
                    if pos.comment and f"AI-{strategy_name}" in pos.comment:
                        # logger.debug(f"Strategy {strategy_name} already has a trade on {symbol}. Skipping.")
                        return False
            
            # Global cap check (optional, if you want to limit TOTAL trades per symbol regardless of strategy)
            # For now, we allow multiple if they are from different strategies, up to a reasonable safety limit
            if len(positions) >= self.config.get('max_open_positions_per_symbol', 5): # Increased default safety cap
                return False

        return True

    def execute_trade(self, symbol, signal_type, trade_params, data, timeframe, strategy_name="Consensus"):
        if not self.can_open_new_trade(symbol, strategy_name):
            logger.info(f"Skipping trade for {symbol} ({strategy_name}): Cooldown or max positions reached.")
            return False
...
<<<<<<< HEAD
                ...
                if current_date > self.last_day_reset:
                    ...
                    if acc_info:
                        ...
                        logger.info(f"New day detected ({current_date}). Daily starting equity reset to: {self.daily_starting_equity}")
                
                # Initialize daily equity if not set (first run)
                if self.daily_starting_equity == 0.0:
                    acc_info = mt5.account_info()
                    if acc_info:
                        self.daily_starting_equity = acc_info.equity
...
<<<<<<< HEAD
                        ...
                        for signal in valid_signals:
                            logger.info(f"Signal from {signal.strategy_name} for {symbol} ({tf_str}): {signal.signal_type}. Executing...")
                            ...
                            trade_params = {
                                ...
                                'sl_pips': signal.metadata.get('sl_pips'),
                                'tp_pips': signal.metadata.get('tp_pips'),
                                'limit_price': signal.metadata.get('limit_price')
                            }
                            executed_ticket = self.execute_trade(symbol, signal.signal_type.lower(), trade_params, data, tf_str, strategy_name=signal.strategy_name)

                            # [NEW] Add to recent signals buffer for UI ONLY if trade executed
                            if executed_ticket:
                                with self.recent_signals_lock:
                                    self.recent_signals.append({
                                        "symbol": symbol,
                                        "type": "Buy" if signal.signal_type.lower() == 'buy' else "Sell",
                                        "strength": signal.metadata.get('confidence', 0.0) if signal.metadata else 0.0,
...
>>>>>>> 3bf0cf4babc04168161ee0889422e8e811a2ac82
    ...
    def get_bot_status_for_ui(self):
        ...
        try:
            ...
            if trades_collection is not None:
                ...
                winning_trades = trades_collection.count_documents({"status": {"$in": ["closed", "closed_auto"]}, "profit_loss": {"$gt": 0}})
                total_closed_trades = trades_collection.count_documents({"status": {"$in": ["closed", "closed_auto"]}})
                
                if total_closed_trades > 0:
                    win_rate = (winning_trades / total_closed_trades) * 100.0
        except Exception as e:
            logger.error(f"Error calculating win rate: {e}")

        # [NEW] Calculate Floating P/L (unrealized profit/loss from open positions)
        floating_pl = 0.0
        if acc_info:
            floating_pl = acc_info.profit  # MT5 provides this directly as unrealized P/L
        ...
        return {
            ...
            "session_pl": round(floating_pl, 2), # Floating P/L from open positions
            "monthly_growth": self.calculate_monthly_growth() # [NEW] Monthly Growth
        }

    def calculate_monthly_growth(self):
        """
        Calculates the Month-to-Date (MTD) percentage growth of the account balance.
        """
        try:
            account_info = mt5.account_info()
            if not account_info:
            ...
            # Fetch deals from start of month to now
            ...
            total_profit = 0.0
            for deal in deals:
                # Filter for entry/exit deals (ignore balance operations if needed, but usually balance ops count for growth)
                # For pure trading growth, we might want to exclude deposits/withdrawals.
                # Assuming deal.profit includes commission and swap
                total_profit += deal.profit + deal.commission + deal.swap
                
            # Starting balance = Current Balance - Total Profit (Profit can be negative)
            starting_balance = current_balance - total_profit
            
            if starting_balance <= 0:
                return 0.0
                
            growth_pct = (total_profit / starting_balance) * 100.0
    ...
    def get_recent_signals_for_ui(self):
        # [NEW] Filter out signals for closed trades
        positions = mt5.positions_get()
        if positions is None:
            # If MT5 connection fails, return existing list without filtering to avoid clearing it
            with self.recent_signals_lock:
        ...
        with self.recent_signals_lock:
            # Keep only signals where the ticket is still in open_tickets
            # Note: This will remove signals immediately upon trade close.
            active_signals = [s for s in self.recent_signals if s.get('ticket') in open_tickets]
            self.recent_signals = active_signals
            return list(self.recent_signals)

    def get_strategy_performance_for_ui(self):
        """
        Aggregates trade results by strategy from MongoDB to calculate win rates.
        Parses strategy name from comment field (format: AI-{StrategyName}-{Timeframe}).
        """
        try:
            trades_collection = MongoDBConnection.get_trades_collection()
            if trades_collection is None:
            ...
            closed_trades = list(trades_collection.find(
                ...
                {"comment": 1, "profit_loss": 1}
            ))
            
            if not closed_trades:
                return []
            
            # Aggregate by strategy parsed from comment
            strategy_stats = {}
            for trade in closed_trades:
                comment = trade.get("comment", "")
                # Parse strategy from comment format: AI-{StrategyName}-{Timeframe}
                # Skip trades that don't have the AI- prefix (e.g., older M5 trades)
                if not comment or not isinstance(comment, str):
                    continue
                    
                parts = comment.split("-")
                if len(parts) < 2 or parts[0] != "AI":
                    continue  # Skip trades without proper AI- format
                
                strategy_name = parts[1]  # Get the strategy name part
                if strategy_name == "M5":
                    strategy_name = "Legacy (M5)"
                
                if strategy_name not in strategy_stats:
                    strategy_stats[strategy_name] = {"wins": 0, "total": 0}
            ...
            # Sort by total trades (most active strategies first)
            performance_data.sort(key=lambda x: x["totalTrades"], reverse=True)
    ...
    def get_strategy_details_for_ui(self):
        """
        Returns detailed strategy performance metrics for the UI.
        Includes win rate, profit factor, recent results, and enabled status.
        """
        try:
            trades_collection = MongoDBConnection.get_trades_collection()
            if trades_collection is None:
            ...
            closed_trades = list(trades_collection.find(
                {"status": {"$in": ["closed", "closed_auto"]}},
                {"comment": 1, "profit_loss": 1, "close_time": 1}
            ))
            
            # Sort by close time for recent results
            closed_trades.sort(key=lambda x: x.get('close_time', datetime.min))

            strategy_stats = {}
            
            # Initialize stats for all active strategies (even if no trades)
            active_strategies = self.config.get('active_strategies', [])
            all_known_strategies = set(active_strategies)
            
            # Also include strategies found in trade history
            for trade in closed_trades:
                comment = trade.get("comment", "")
                if comment and isinstance(comment, str) and comment.startswith("AI-"):
                    parts = comment.split("-")
                    if len(parts) >= 2:
                        all_known_strategies.add(parts[1])

            for strategy_name in all_known_strategies:
                if strategy_name == "M5":
                    strategy_name = "Legacy (M5)"
            ...
            for trade in closed_trades:
                ...
                # 1. Try to get strategies from the 'strategies' field (new trades)
                db_strategies = trade.get("strategies")
                if db_strategies and isinstance(db_strategies, list) and len(db_strategies) > 0:
                    strategies_involved = db_strategies
                
                # 2. Fallback to comment parsing (old trades or missing field)
                if not strategies_involved:
                    comment = trade.get("comment", "")
                    if comment and isinstance(comment, str) and comment.startswith("AI-"):
                        parts = comment.split("-")
                        if len(parts) >= 2:
                            strategies_involved = [parts[1]]
                
                # If still no strategies found, skip
                if not strategies_involved:
                    continue

                for strategy_name in strategies_involved:
                    # Apply M5 renaming logic
                    if strategy_name == "M5":
                        strategy_name = "Legacy (M5)"
                    
                    # Initialize if not exists (dynamic addition)
                    if strategy_name not in strategy_stats:
                        strategy_stats[strategy_name] = {
                            "wins": 0, 
                            "losses": 0,
            ...
            for name, stats in strategy_stats.items():
                ...
                details.append({
                    ...
                    "name": name,
                    "enabled": name in active_strategies,
                    "winRate": round(win_rate, 1),
                    "totalTrades": total,
            ...
            details.sort(key=lambda x: x["totalTrades"], reverse=True)
            return details
    ...
    def toggle_strategy(self, strategy_id, enabled):
        """
        Enables or disables a strategy by updating the configuration file.
        """
        with self.config_lock:
            try:
                active_strategies = self.config.get('active_strategies', [])
                ...
                self.config['active_strategies'] = active_strategies
    ...
    def get_all_trades_for_ui(self):
        """
        Returns a combined list of active and closed trades formatted for the Advanced Trade Table.
        Structure:
        [
          {
            "symbol": "BTCUSD",
            ...
            "size": 0.1,
            "entryPrice": 36540.20,
            "currentPrice": 36685.00, # Or Exit Price for closed
            "pl": 1448.00,
            "strategy": "SMC",
        ...
        # 1. Get Active Trades (Open Positions)
        try:
            positions = mt5.positions_get()
            if positions:
                for pos in positions:
                    if pos.magic == 123456:
                        ...
                        # Determine strategy from comment or DB (simplified here to comment parsing for speed, 
                        # or could query DB if needed, but comment is usually reliable for active)
            ...
            logger.error(f"Error fetching open positions for all_trades: {e}")

        # 2. Get Closed Trades (History) from MongoDB
        try:
            trades_collection = MongoDBConnection.get_trades_collection()
            if trades_collection is not None:
                # Fetch last 200 closed trades to avoid overwhelming the UI
                # Adjust limit as needed or add pagination support if the table supports it
                cursor = trades_collection.find({"status": {"$in": ["closed", "closed_auto", "closed_unknown"]}}).sort("exit_time", DESCENDING).limit(200)
                
                for trade in cursor:
                    # Strategy parsing
                    strategies_val = trade.get("strategies", [])
                    strategy_name = "Unknown"
                    if isinstance(strategies_val, list) and len(strategies_val) > 0:
                        strategy_name = strategies_val[0]
                    elif isinstance(strategies_val, str):
                        strategy_name = strategies_val
                    
                    # Fallback to comment if strategy field is empty/unknown
                    if strategy_name == "Unknown" or not strategy_name:
                         comment = trade.get("comment", "")
                         if comment:
                            parts = comment.split('-')
                            if len(parts) >= 2:
                    ...
                    all_trades.append({
                        ...
                        "type": str(trade.get("signal", "N/A")).upper(),
                        "size": float(trade.get("lot_size", 0.0)),
                        "entryPrice": float(trade.get("entry_price", 0.0)),
                        "currentPrice": float(trade.get("exit_price", 0.0)), # Exit price for closed
                        "pl": float(trade.get("profit_loss", 0.0)),
                        "strategy": strategy_name,
                        "status": "Closed"
                    })
        except Exception as e:
            logger.error(f"Error fetching closed trades for all_trades: {e}")
