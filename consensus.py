...
<<<<<<< HEAD
    ...
    def _get_ml_confidence(self, symbol: str, signal_type: str, data: pd.DataFrame) -> float:
        """
        Calculates the ML model's confidence (probability) for the signal.
        """
        if not self.ml_validator or not self.bot_instance:
            return 0.0

        try:
            # 1. Extract Features
            features = extract_ml_features(symbol, data, signal_type.lower(), self.bot_instance)
            
            # 2. Get Probability
            # predict_proba returns [[prob_0, prob_1]] for single sample
            probs = self.ml_validator.predict_proba(symbol, features, signal_type.lower())
            
            if probs is not None and len(probs) > 0:
                # Return probability of class 1 (Trade)
                return float(probs[0][1])
                
        except Exception as e:
            logger.error(f"Error calculating ML confidence for {symbol}: {e}")
        
        return 0.0

    def get_valid_signals(self, data: pd.DataFrame, symbol: str, timeframe: str) -> List[TradeSignal]:
        """
        Returns a list of valid signals from all strategies, individually validated.
        Skips consensus aggregation.
        """
        if data is None or data.empty:
            return []

        # 1. Detect Regime (Optional context)
        regime = "unknown"
        if self.regime_filter:
            regime = self.regime_filter.get_regime(data)
            # logger.debug(f"Market Regime for {symbol}: {regime}")
        ...
        for strategy in self.strategies:
            try:
                ...
                # Apply ADX Filter
                if not self._check_adx_filter(signal_obj, adx_context, symbol):
                    # logger.debug(f"Signal {signal_obj.strategy_name} {signal_obj.signal_type} filtered by ADX.")
                    continue

                # Apply ML Validation
                if self.ml_validator:
                    # Calculate and store actual confidence score
                    confidence_score = self._get_ml_confidence(symbol, signal_obj.signal_type, data)
                    if signal_obj.metadata is None: signal_obj.metadata = {}
                    signal_obj.metadata['confidence'] = confidence_score
                    
                    # Optional: Log high confidence signals
                    # if confidence_score > 0.7:
                    #     logger.info(f"High ML Confidence ({confidence_score:.2f}) for {signal_obj.strategy_name} on {symbol}")

                    if not self._validate_with_ml(symbol, signal_obj.signal_type, data):
                        logger.info(f"ML Vetoed {signal_obj.strategy_name} {signal_obj.signal_type} for {symbol}")
                        continue

                # If passed all checks, add to list
                valid_signals.append(signal_obj)
