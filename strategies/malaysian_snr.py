...
class MalaysianSnRStrategy:
    ...
    """
    def __init__(self, name="MalaysianSnR", window=100, threshold=0.005, miss_period=5, enable_rejection=True, enable_miss=True, enable_engulfing=True, trend_filter_enabled=True, trend_ema_period=50):
        """
        ...
        :param trend_filter_enabled: If True, filters trades against the trend.
        :param trend_ema_period: Period for the EMA trend filter.
        """
        self.name = name
        self.window = window
    ...
    def set_config(self, config: dict):
        ...
        self.trend_filter_enabled = config.get('snr_trend_filter_enabled', self.trend_filter_enabled)
        self.trend_ema_period = config.get('snr_trend_ema_period', self.trend_ema_period)
        logger.debug(
            ...
            f"TrendFilter={self.trend_filter_enabled}, TrendEMA={self.trend_ema_period}"
        )
    ...
    def identify_levels(self, data: pd.DataFrame) -> tuple[list, list]:
        ...
        Includes Significance Filter: Price must move away by > 1.5 * ATR.
        """
        resistance_levels = []
        support_levels = []
    ...
    def get_signal(self, data: pd.DataFrame, symbol: str = None, timeframe: str = None, return_features=False):
        ...
        trend_ema = self._calculate_ema(df, self.trend_ema_period).iloc[-1]
        ...
        if fresh_resistances:
            ...
            if abs(current_price - level_price) <= self.threshold * current_price:
                # Base strength is 0. Proximity alone is not enough.
                
                # Trend Filter Check
                if self.trend_filter_enabled and not is_downtrend:
                    # Counter-trend trade. Reduce score or block?
                    # Let's just NOT add the trend bonus.
                    pass
                else:
                    sell_signal_strength += 0.2 # Bonus for trading with trend

                if self.enable_rejection and self._is_rejection(level_price, last_candle, is_resistance=True):
                    sell_signal_strength += 0.5 # Strong confirmation
                
                if self.enable_miss and self._is_miss(nearest_res[0], level_price, df):
                    sell_signal_strength += 0.1 # Minor confirmation
                
                if self.enable_engulfing and self._is_engulfing(df, 'sell'):
                    sell_signal_strength += 0.3 # Good confirmation
        ...
        if fresh_supports:
            ...
            if abs(current_price - level_price) <= self.threshold * current_price:
                # Trend Filter Check
                if self.trend_filter_enabled and not is_uptrend:
                    pass
                else:
                    buy_signal_strength += 0.2 # Bonus for trading with trend

                if self.enable_rejection and self._is_rejection(level_price, last_candle, is_resistance=False):
                    buy_signal_strength += 0.5 # Strong confirmation
                
                if self.enable_miss and self._is_miss(nearest_sup[0], level_price, df):
                    buy_signal_strength += 0.1 # Minor confirmation
                
                if self.enable_engulfing and self._is_engulfing(df, 'buy'):
                    buy_signal_strength += 0.3 # Good confirmation
        ...
        elif sell_signal_strength > buy_signal_strength and sell_signal_strength >= 0.5: # Require at least 0.5 strength
